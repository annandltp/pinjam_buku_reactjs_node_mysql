...
export default function Datatable1(props) {
    ...
    const [mahasiswaId, setMahasiswaId] = useState(0);
    ...
    const headers = [
        ...
        { name: "NIM", field: "nim", sortable: true },
        { name: "Nama", field: "nama", sortable: true },
        { name: "Fakultas", field: "fakultas", sortable: false },
    ...
    async function onEdit(event, set_id) {
        ...
        history.push("/mahasiswa/edit/" + set_id + "");
    }
    ...
    useEffect(() => {
        setTimeout(() => {
            async function getData() {
                setLoading(true);
                const getURL = `${api}mahasiswa/get`;
    ...
    async function onDelete(set_id) {
        if(confirm){
            ...
            const getURL = `http://localhost:4000/mahasiswa/delete/${set_id} `;
    ...
    useEffect(() => {
        if(confirm){
            const mahasiswa_id = `${mahasiswaId}`            
            onDelete(mahasiswa_id);
        }
    }, [confirm]);
